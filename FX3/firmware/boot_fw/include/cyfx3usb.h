/*
 ## Cypress FX3 Boot Firmware Header (cyfx3usb.h)
 ## ===========================
 ##
...
/** \brief List of USB 3.0 Link Power States.

    **Description**\n
    This is a partial list of the possible USB 3.0 Link Power states. This structure only
    comprehends the steady states like U0, U1, U2; and does not include transient states
    which may be hit while moving from one steady state to another (like polling or recovery).
 */
...
/** \brief Retrieve the current state of the specified endpoint.
    ...
    **Return value**\n
    * CY_FX3_BOOT_SUCCESS - On Success.\n
    * CY_FX3_BOOT_ERROR_NULL_POINTER - If both isNak and isStall parameters are NULL.
 */
extern CyFx3BootErrorCode_t
...
/** \brief Get the current USB 3.0 link power state.

    **Description**\n
    This function retrieves the current USB 3.0 link power state, so that the application can decide
 ...
 */
extern CyU3PUsbLinkPowerMode
CyFx3BootUsbGetLinkPowerState (
        void);

/** \brief Trigger a change to the desired USB 3.0 link power state.

    **Description**\n
    This function triggers a USB 3.0 link power state change from U1/U2/U3 to U0; or from U0 to U1/U2.
    This function will do nothing if the USB 3.0 link is not active; or if it is in a polling or recovery
    state.
    ...
    **\see
    *\see CyU3PUsbLinkPowerMode
    *\see CyFx3BootUsbGetLinkPowerState
 */
extern void
CyFx3BootUsbSetLinkPowerState (
        CyU3PUsbLinkPowerMode mode                      /**< USB 3.0 link power mode to switch to. */
        );

/** \brief Trigger a USB 2.0 resume signal.

    **Description**\n
    This function triggers a USB 2.0 resume signal to initiate a remote wake. The resume signal will
    be driven for about 20 ms if the link is suspended, and for about 50 us if the link is in the
    LPM-L1 state. The API does not check whether the link is actually suspended. It is the caller's
...
/** \brief Handler that manages USB link state changes.

    **Description**\n
    This is an utility function that can be called periodically to have all USB 3.0 and 2.0 link states
    to be managed by the driver. Calling this function is equivalent to calling the CyFx3BootUsbCheckUsb3Disconnect,
    CyFx3BootUsbEp0StatusCheck and CyFx3BootUsbSendCompliancePatterns functions.
