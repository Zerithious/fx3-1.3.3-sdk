/*
 ## Cypress FX3 Boot Firmware Source (cyfx3i2c.c)
 ## ===========================
 ##
        ...
        uint32_t retryCount)
{
    ...
    timeout = CY_FX3_BOOT_ERROR_TIMEOUT;
    ...
    do 
    {
        ...
        if (status == CY_FX3_BOOT_SUCCESS)
        {
            /* All the data has been written to the FIFO. Wait until the FIFO is drained. */
