...
OBJASM=$(SOURCE_ASM:%.S=$(CYCONFIG)/%.o)
OBJECT+=$(OBJASM:%.s=$(CYCONFIG)/%.o)

CYU3POUTDIR=$(CYU3ROOT)/bin/fw/$(CYCONFIG)
...
##
checkplatform ::
	@if [ ! -f $(CYU3ROOT)/build/fwconfig/$(CYCONFIG).mak ]; then          		\
	  echo " ";                                                                	\
	  echo "Configuration '$(CYCONFIG)' is not valid." ;                       	\
	  echo " ";                                                                	\
	  echo "Valid configurations are:" ;                                       	\
	  echo "*****************************************************************";	\
	  cd $(CYU3ROOT)/build/fwconfig; ls -C *.mak | sed -e "s-.mak-    -g"; 		\
	  echo "*****************************************************************";	\
	  echo " " ;                                                               	\
...
cleanallconfigs:
	@ ls $(CYU3ROOT)/build/fwconfig | sed -n s/.mak$$//p | xargs -i rm -rf {}
