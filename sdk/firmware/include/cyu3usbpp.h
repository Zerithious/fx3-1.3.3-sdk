/*
 ## Cypress FX3 Core Library Header (cyu3usbpp.h)
 ## ===========================
 ##
...
/* Check whether the FX3 device can connect to the USB host. This is allowed if VBus detection is disabled,
   or if VBus is detected.
 */
#define CyU3PUsbCanConnect()            \
    ((glUibDeviceInfo.vbusDetectMode == 0) || ((GCTL->iopower & glUibDeviceInfo.vbusDetectMode) != 0))

/* Set the NAK-ALL bit in a safe manner. */
#define CyU3PUsbSetNakAll()                                                                                     \
{                                                                                                               \
    if (glUibDeviceInfo.usbSpeed == CY_U3P_SUPER_SPEED)                                                         \
        ...
        UIB->dev_cs = (UIB->dev_cs & ~CY_U3P_UIB_SETUP_CLR_BUSY) | CY_U3P_UIB_NAKALL;                           \
}

/* Clear the NAK-ALL bit in a safe manner. */
#define CyU3PUsbClearNakAll()                                                                                   \
{                                                                                                               \
    if (glUibDeviceInfo.usbSpeed == CY_U3P_SUPER_SPEED)                                                         \
        USB3PROT->prot_cs &= ~(CY_U3P_UIB_SS_SETUP_CLR_BUSY | CY_U3P_UIB_SS_NRDY_ALL);                          \
    else                                                                                                        \
        UIB->dev_cs &= ~(CY_U3P_UIB_SETUP_CLR_BUSY | CY_U3P_UIB_NAKALL);                                        \
}
...
#define CY_U3P_USB_MSC_FS_INTFDESC_LENGTH	(0x17)
#define CY_U3P_USB_MSC_HS_INTFDESC_LENGTH	(0x17)
#define CY_U3P_USB_MSC_SS_INTFDESC_LENGTH       (0x23)

#define CY_U3P_USB_MTP_FS_INTFDESC_LENGTH       (0x1E)
#define CY_U3P_USB_MTP_HS_INTFDESC_LENGTH       (0x1E)
#define CY_U3P_USB_MTP_SS_INTFDESC_LENGTH       (0x30)

#define CY_U3P_USB_UAS_FS_INTFDESC_LENGTH       (0x35)
#define CY_U3P_USB_UAS_HS_INTFDESC_LENGTH       (0x35)
#define CY_U3P_USB_UAS_SS_INTFDESC_LENGTH       (0x4D)
...
typedef struct
{
    ...
    CyBool_t            inUsbTestMode;                  /* Whether the device is in USB 2.0 test mode. */
    CyBool_t            resumeFromTimer;                /* Whether USB 2.0 resume function is called from timer. */
    CyBool_t            l1EntrySeen;                    /* Whether USB 2.0 L1 entry interrupt has been seen. */
} CyU3PUsbDevStat;


typedef struct
{
    uint8_t     *usbDevDesc_p;			        /* Pointer to USB 2.0 device descriptor. */
    uint8_t	*usbSSDevDesc_p;		        /* Pointer to USB 3.0 device descriptor. */
    uint8_t     *usbDevQualDesc_p;		        /* Pointer to device qualifier descriptor. */
    uint8_t     *usbConfigDesc_p;		        /* Pointer to active config descriptor (USB 2.0) */
    uint8_t     *usbOtherSpeedConfigDesc_p;	        /* Pointer to other speed configuration descriptor. */
    uint8_t     *usbHSConfigDesc_p;                     /* Pointer to HI-SPEED configuration descriptor. */
    uint8_t     *usbFSConfigDesc_p;                     /* Pointer to FULL SPEED configuration descriptor. */
    uint8_t     *usbSSConfigDesc_p;                     /* Pointer to SUPER SPEED configuration descriptor. */
    uint8_t     *usbStringDesc_p[CY_U3P_MAX_STRING_DESC_INDEX + 1]; /* Array of pointers to string descriptors. */
    uint8_t     *usbIntfDescHS_p;                       /* Pointer to HI-SPEED interface descriptor. Unused. */
    uint8_t     *usbIntfDescFS_p;                       /* Pointer to FULL SPEED interface descriptor. Unused. */
    uint8_t     *usbIntfDescSS_p;                       /* Pointer to SUPER SPEED Interface Descriptor. Unused. */
    uint8_t     *usbSSBOSDesc_p;                        /* Pointer to BOS descriptor. */
...
extern CyBool_t           glUsbSSCDisable;              /* Flag indicating that SSC is disabled. */
