/*
 ## Cypress FX3 DMA Driver Source (cyu3channel.c)
 ## ===========================
 ##
        ...
        uint32_t waitOption)
{
    ...
    if ((handle->startSig != CY_U3P_DMA_CHANNEL_START_SIG) || (handle->endSig != CY_U3P_DMA_CHANNEL_END_SIG))
    {
        ...
        CyU3PDmaChannelConfig_t *config)
{
    ...
    if (status == CY_U3P_SUCCESS)
    {
        ...
        /* Set the channel signatures. */
        handle->startSig = CY_U3P_DMA_CHANNEL_START_SIG;
        ...
        CyU3PDmaChannel *handle)
{
    ...
    /* Destroy the locks, and clear the signatures. */
    CyU3PMutexDestroy (&(handle->lock));
    CyU3PEventDestroy (&(handle->flags));
    handle->state    = CY_U3P_DMA_NOT_CONFIGURED;
    handle->startSig = 0;
        ...
        )
{
    ...
    /* We need to suspend ACTIVE sockets only. */
    if (handle->state == CY_U3P_DMA_ACTIVE)
    {
        /* Update consumer socket option. */
        if (handle->type != CY_U3P_DMA_TYPE_MANUAL_IN)
        {
            /* Instruct the consumer to suspend, and wait until it suspends. */
            handle->usbConsSusp = 1;
            CyU3PDmaUpdateSocketSuspendOption (handle->consSckId, CY_U3P_DMA_SCK_SUSP_CUR_BUF);
        ...
        CyU3PDmaChannel *handle)
{
    ...
    if (handle->usbConsSusp != 0)
    {
        handle->usbConsSusp = 0;
        CyU3PDmaUpdateSocketSuspendOption (handle->consSckId, handle->consSusp);
        CyU3PDmaUpdateSocketResume (handle->consSckId, handle->consSusp);
    }
        ...
        CyBool_t        isConsResume)
{
    ...
    /* Do not resume the socket if it is suspended internally for USB device work-around. */
    if ((isConsResume == CyTrue) && (handle->usbConsSusp == 0))
    {
        /* Resume the consumer socket. If the socket was
         * asked to be suspended after the current buffer,
         * suspend state has to be reverted to SUSP_NONE. */
        if (handle->consSusp == CY_U3P_DMA_SCK_SUSP_CUR_BUF)
        {
            handle->consSusp = CY_U3P_DMA_SCK_SUSP_NONE;
        }
...
#ifdef CYU3P_DISABLE_ERROR_CHECK
    if (handle->state == CY_U3P_DMA_ERROR)
    {
        status = CY_U3P_ERROR_DMA_FAILURE;
    }
