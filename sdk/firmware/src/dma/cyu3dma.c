/*
 ## Cypress FX3 DMA Driver Source (cyu3dma.c)
 ## ===========================
 ##
...
#define CY_U3P_DMA_THREAD_STACK         (0x400)         /* DMA driver thread stack size is 1 KB. */
#define CY_U3P_DMA_THREAD_PRIORITY      (2)             /* DMA driver thread priority. */
#define CY_U3P_DMA_QUEUE_SIZE           (32)            /* Queue size in terms of entries. */
#define CY_U3P_DMA_MSG_SIZE             (16)            /* Size of each message in bytes. */
        ...
        uint32_t *msg)
{
    ...
    else /* Producer socket events */
    {
        ...
        if (status & CY_U3P_TRANS_DONE)
        {
            /* Check if all the buffers have already been discarded or committed. If so, we can generate
               the XFER_CPLT event from here. */
            CyU3PDmaDscrGetConfig (h->currentProdIndex, &dscr);
            if ((h->currentProdIndex == h->activeProdIndex) && ((dscr.size & CY_U3P_BUFFER_OCCUPIED) == 0))
            {
                h->state = CY_U3P_DMA_CONFIGURED;

                /* Notify any waiting threads and send the event callback. */
                CyU3PEventSet (&(h->flags), CY_U3P_DMA_CB_XFER_CPLT, CYU3P_EVENT_OR);
                if ((h->cb) && (h->notification & CY_U3P_DMA_CB_XFER_CPLT))
                {
                    h->cb (h, CY_U3P_DMA_CB_XFER_CPLT, 0);
        ...
        uint32_t input)
{
    uint8_t  task;
    uint32_t status;
    uint32_t msg[CY_U3P_DMA_MSG_SIZE / sizeof (uint32_t)];
    CyU3PDmaChannel *h;
        ...
        void)
{
    ...
    CyU3PQueueCreate (&glDmaQueue, CY_U3P_DMA_MSG_SIZE / sizeof (uint32_t), ptr, 
    ...
    CyU3PSystemRegisterDriver (&glDmaThread);
}
...
{
    CyBool_t ret = CyFalse;

    if ((handle != NULL) && (handle->startSig == CY_U3P_DMA_CHANNEL_START_SIG) &&
            (handle->endSig == CY_U3P_DMA_CHANNEL_END_SIG))
    {
        ret = CyTrue;
    }
        ...
        CyU3PDmaMultiChannel *handle)
{
    ...
    if ((handle != NULL) && (handle->startSig == CY_U3P_DMA_MULTICHN_START_SIG) &&
            (handle->endSig == CY_U3P_DMA_CHANNEL_END_SIG))
    {
        ret = CyTrue;
    }
...
CyU3PDmaUsbInEpGetChannel (
        uint8_t ep)
{
    CyU3PDmaChannel *handle_p = NULL;

    if ((ep < CY_U3P_DMA_UIB_NUM_SCK) && (CyU3PDmaChannelIsValid (glDmaUIBSocketCtrl[ep].singleHandle)))
        handle_p = glDmaUIBSocketCtrl[ep].singleHandle;
    return handle_p;
}
...
CyU3PDmaUsbInEpGetMultiChannel (
        uint8_t ep)
{
    CyU3PDmaMultiChannel *handle_p = NULL;

    if ((ep < CY_U3P_DMA_UIB_NUM_SCK) && (CyU3PDmaMultiChannelIsValid (glDmaUIBSocketCtrl[ep].multiHandle)))
        handle_p = glDmaUIBSocketCtrl[ep].multiHandle;
    return handle_p;
}
