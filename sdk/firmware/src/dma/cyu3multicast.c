/*
 ## Cypress FX3 DMA Driver Source (cyu3multicast.c)
 ## ===========================
 ##
        ...
        uint16_t              consIndex)
{
    ...
    {
        if (consIndex < chHandle->validSckCount)
        {
            if (((chHandle->state == CY_U3P_DMA_ACTIVE) || (chHandle->state == CY_U3P_DMA_IN_COMPLETION))
                    && (chHandle->consDisabled[consIndex] == 0))
            {
                /* Mark this consumer as disabled. */
                chHandle->consDisabled[consIndex] = 1;

                /* Find out how many buffers are waiting for consume event from the disabled consumer. */
                pendCount = chHandle->count;
                for (sck = 0; sck < chHandle->validSckCount; sck++)
                {
                    if ((sck != consIndex) && (chHandle->bufferCount[sck] < pendCount))
                        pendCount = chHandle->bufferCount[sck];
                }

                /* Generate consume events for any buffers that are waiting for this socket only. */
                if (pendCount < chHandle->bufferCount[consIndex])
                {
                    for (sck = 0; sck < (chHandle->bufferCount[consIndex] - pendCount); sck++)
                    {
                        /* Mark the producer descriptor free. */
                        index = chHandle->commitProdIndex;
                        CyU3PDmaDscrGetConfig (index, &dscr);
                        chHandle->commitProdIndex = (dscr.chain >> CY_U3P_WR_NEXT_DSCR_POS);
                        dscr.size &= CY_U3P_BUFFER_SIZE_MASK;
                        CyU3PDmaDscrSetConfig (index, &dscr);
                        CyU3PSysBarrierSync ();

                        /* Send a consume event to the producer socket and to the channel callback. */
                        CyU3PDmaSocketSendEvent (chHandle->prodSckId[0], chHandle->commitProdIndex, CyFalse);
                        if ((chHandle->cb != NULL) && (chHandle->notification & CY_U3P_DMA_CB_CONS_EVENT))
                            chHandle->cb (chHandle, CY_U3P_DMA_CB_CONS_EVENT, 0);

                        /* Advance to the appropriate consumer descriptor. */
                        CyU3PDmaDscrGetConfig (chHandle->activeConsIndex[consIndex], &dscr);
                        chHandle->activeConsIndex[consIndex] = (dscr.chain & CY_U3P_RD_NEXT_DSCR_MASK);
                    }

                    /* If all the expected data has already been produced, send the transfer complete notification. */
                    if ((chHandle->state == CY_U3P_DMA_IN_COMPLETION) && (pendCount == 0))
                    {
                        chHandle->state = CY_U3P_DMA_CONFIGURED;
                        CyU3PEventSet (&(chHandle->flags), CY_U3P_DMA_CB_XFER_CPLT, CYU3P_EVENT_OR);
                        CyU3PSysBarrierSync ();
                        if ((chHandle->cb != NULL) && (chHandle->notification & CY_U3P_DMA_CB_XFER_CPLT))
                            chHandle->cb (chHandle, CY_U3P_DMA_CB_XFER_CPLT, 0);
                    }
                }

                /* Mark that this socket has no more pending consume events. */
                chHandle->bufferCount[consIndex] = 0;
            }
            else
