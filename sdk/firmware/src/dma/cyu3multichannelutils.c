/*
 ## Cypress FX3 DMA Driver Source (cyu3multichannelutils.c)
 ## ===========================
 ##
        ...
        )
{
    ...
    /* We cannot handle a channel with multiple USB IN consumer sockets. */
    if ((handle->type != CY_U3P_DMA_TYPE_AUTO_MANY_TO_ONE) &&
            (handle->type != CY_U3P_DMA_TYPE_MANUAL_MANY_TO_ONE))
    {
        CyU3PMutexPut (&(handle->lock));
    ...
    if (handle->state == CY_U3P_DMA_ACTIVE)
    {
        ...
        CyU3PDmaUpdateSocketSuspendOption (handle->consSckId[0], CY_U3P_DMA_SCK_SUSP_CUR_BUF);
        ...
        CyU3PDmaMultiChannel *handle)
{
    ...
    if (handle->usbConsSusp != 0)
    {
        ...
        CyU3PDmaUpdateSocketSuspendOption (handle->consSckId[0], handle->consSusp);
        CyU3PDmaUpdateSocketResume (handle->consSckId[0], handle->consSusp);
    }
