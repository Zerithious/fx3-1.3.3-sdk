/*
 ## Cypress FX3S Storage Driver Source (cyu3cardmgr_fx3.c)
 ## ===========================
 ##
...
/* Timer Period for SIB Write Transactions */
extern uint32_t glSibWriteTimerPeriod;
        ...
        CyBool_t halfDiv)
{
    ...
    if (halfDiv)
        clkValue |= CY_U3P_GCTL_SIBCLK_HALFDIV;
        ...
        uint8_t portId)
{
    ...
    /* Avoid stopping Single Block Data Read and Write */
    if ((SIB->sdmmc[portId].cmd_idx != CY_U3P_SD_MMC_CMD24_WRITE_BLOCK) &&
            (SIB->sdmmc[portId].cmd_idx != CY_U3P_SD_MMC_CMD17_READ_SINGLE_BLOCK))
    {
        ...
        uint16_t numBlks)
{
    ...
    if (!isRead)
    {
        ...
        CyU3PTimerModify (&glSibCtxt[portId].writeTimer, glSibWriteTimerPeriod, 0);
        ...
        uint32_t blkAddr)
{
    ...
    /* Update the active Logical Unit of the card on which operations are being performed. */
    glSibCtxt[portId].activeUnitId = unit;
        ...
        uint8_t lowVoltage)
{
    ...
    /* Reset Clocking */
    CyU3PCardMgrSetClockFreq (portId, CY_U3P_CLOCK_DIVIDER_400, CyFalse);
        ...
        uint8_t lowVoltageSwitch)
{
    ...
    if (CyFx3DevIOIsSib1BitWide (portId))
    ...
    else
    {
        ...
        if (glCardCtxt[portId].cardCapability & CY_U3P_SDIO_CARD_CAPABLITY_LSC)
        {
            ...
            if ((data & CY_U3P_SDIO_CARD_CAPABLITY_4BLS) && (!CyFx3DevIOIsSib1BitWide (portId)))
            {
    ...
    CyU3PCardMgrSetClockFreq (portId, clkDiv, CyFalse);
...
/* Suspend and Resume the function */
CyU3PReturnStatus_t
...
/* Function to read particular number of bytes of data from SDIO device */
/* CDT 177164 Fix: Data Transfer of less than 16 bytes */
CyU3PReturnStatus_t
        ...
        )
{
    ...
    if ((functionNumber > 7) || (registerAddr > 0x1FFFF) || (transferCount >= 0x10) || (transferCount == 0))
    {
        return CY_U3P_ERROR_BAD_ARGUMENT;
    }

    /* Loop till error or read/write expected number of bytes */
    while (counter < transferCount)
    {               
        status = CyU3PSdioByteReadWrite (portId, functionNumber, isWrite, 0, registerAddr, &data_p[counter]);
        if (status != CY_U3P_SUCCESS)
        {
            break;
        }

        if (opCode == CY_U3P_SDIO_RW_ADDR_AUTO_INCREMENT)
        {
            registerAddr++;
        }              
        ...
        uint16_t sckId)
{
    ...
    {
        /* CDT 177164 Fix: Reject Transfer of less than 16 bytes data through this API */
        if (transferCount < 16)
        {
    ...
    if ((data & 0xCF) != 0)
    {
        ...
        /* Sending the appropriate error code back. Masking the IO_CURRENT_STATE field in the response. */
        ...
        uint8_t portId)
{
    ...
    if (((glSibBuffer[1] & 0x04) > 0) && (!CyFx3DevIOIsSib1BitWide (portId)))
    {
        /* Send the APP_CMD(CMD55) command. Expect R1 response. */
        CyU3PCardMgrSendCmd (portId, CY_U3P_SD_MMC_CMD55_APP_CMD, CY_U3P_SD_MMC_R1_RESP_BITS,
                (glCardCtxt[portId].cardRCA << 16), 0);
    ...
    if ((glSibBuffer[0] & 0x0F) > 0)
    {
                ...
                (glCardCtxt[portId].opVoltage == CY_U3P_SIB_VOLTAGE_LOW))
        {
            if ((glSibIntfParams[portId].useDdr) && ((glSibBuffer[13] & 0x10) != 0))
            {
                ...
                CyU3PCardMgrSetClockFreq (portId, CY_U3P_CLOCK_DIVIDER_104M, CyFalse);
                glCardCtxt[portId].uhsIOpMode = CY_U3P_SIB_UHS_I_DDR50;
        ...
        if (!freqSet)
        {
                    ...
                    ((glSibBuffer[13] & 0x02) > 0))
            {
                ...
                CyU3PCardMgrSetClockFreq (portId, CY_U3P_CLOCK_DIVIDER_52M, CyFalse);
            }
        }
    }
        ...
        uint8_t portId)
{
    ...
    if (glSibIntfParams[portId].maxFreq >= CY_U3P_SIB_FREQ_26MHZ)
        CyU3PCardMgrSetClockFreq (portId, CY_U3P_CLOCK_DIVIDER_26M, CyFalse);
    else
        CyU3PCardMgrSetClockFreq (portId, CY_U3P_CLOCK_DIVIDER_20M, CyFalse);
        ...
        uint8_t portId)
{
    ...
    if ((busWidth == CY_U3P_CARD_BUS_WIDTH_1_BIT) && (!CyFx3DevIOIsSib1BitWide (portId)))
    {
        ...
        else
        {
            ...
            /* Clear the error, so that we can go forward in one bit mode. */
        ...
        uint8_t portId)
{
    ...
    glSibCtxt[portId].activeUnitId = 0;
