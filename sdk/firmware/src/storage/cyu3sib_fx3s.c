/*
 ## Cypress FX3S Storage Driver Source (cyu3sib_fx3s.c)
 ## ===========================
 ##
...
uint32_t glSibWriteTimerPeriod = CY_U3P_SIB_WRITE_DATA_TIMEOUT; /* SIB Write Timer Interval */
...
/* Funtion to modify Timer for SIB Write Transaction at runtime */
void
CyU3PSibWriteTimerModify (
        uint32_t newTimerVal)
{   
    glSibWriteTimerPeriod = newTimerVal;

    CyU3PTimerStop (&glSibCtxt[0].writeTimer);    
    CyU3PTimerModify (&glSibCtxt[0].writeTimer, glSibWriteTimerPeriod, 0);

    CyU3PTimerStop (&glSibCtxt[1].writeTimer);    
    CyU3PTimerModify (&glSibCtxt[1].writeTimer, glSibWriteTimerPeriod, 0);    
}
        ...
        uint8_t            *dataLen)
{
    if (!CyU3PIsSibActive ())
        return CY_U3P_ERROR_NOT_STARTED;
    if ((portId > 1) || (dataBuffer == 0) || (regType > CY_U3P_SD_REG_CSD) || (dataLen == 0))
        return CY_U3P_ERROR_BAD_ARGUMENT;
    ...
    switch (regType)
    {
        case CY_U3P_SD_REG_OCR:
            CyU3PMemCopy (dataBuffer, (uint8_t *)&glCardCtxt[portId].ocrRegData, 4);
            *dataLen = 4;
            break;
        case CY_U3P_SD_REG_CID:
            CyU3PMemCopy (dataBuffer, glCardCtxt[portId].cidRegData, CY_U3P_SD_REG_CID_LEN);
            *dataLen = CY_U3P_SD_REG_CID_LEN;
            break;
        case CY_U3P_SD_REG_CSD:
            CyU3PMemCopy (dataBuffer, glCardCtxt[portId].csdRegData, CY_U3P_SD_REG_CSD_LEN);
            *dataLen = CY_U3P_SD_REG_CSD_LEN;
        default:
            break;
    }
        ...
        uint8_t portId )
{
    ...
    if (glCardCtxt[portId].cardType == CY_U3P_SIB_DEV_NONE)
    {
        ...
        status = CyU3PTimerCreate (&glSibCtxt[portId].writeTimer, glSibCtxt[portId].writeTimerCbk, 0,
                glSibWriteTimerPeriod, 0, CYU3P_NO_ACTIVATE);
        if (status != CY_U3P_SUCCESS)
        {
        ...
        uint8_t portId )
{
    ...
    /* Revert to default timer period once SIB got de-initialized */
    glSibWriteTimerPeriod = CY_U3P_SIB_WRITE_DATA_TIMEOUT;
}
        ...
        void)
{
    ...
    /* Verify that the S-port is supported by the device under test. */
    ...
    CyU3PCardMgrSetClockFreq (0, CY_U3P_CLOCK_DIVIDER_400, CyFalse);
    CyU3PCardMgrSetClockFreq (1, CY_U3P_CLOCK_DIVIDER_400, CyFalse);
    ...
    {
        /* The port is not configured for SD/MMC */
        return CY_U3P_ERROR_NOT_CONFIGURED;
    }
