/*
 ## Cypress FX3 Device Firmware Source (cyfx3stor.c)
 ## ===========================
 ##
        ...
        CyU3PStorDriverFunction entry_p)
{
    if (entry_p != 0)
    {
        ...
        CyU3PEventSet (&glSibEvent, CY_U3P_SIB_EVT_DRVENTRY, CYU3P_EVENT_OR);
    }
}
        ...
        uint32_t threadInput)
{
    ...
    while (1)
    {
        /* Wait until the driver function has been registered, and transfer control to that. */
        status = CyU3PEventGet (&glSibEvent, CY_U3P_SIB_EVT_DRVENTRY, CYU3P_EVENT_OR_CLEAR, &flag, CYU3P_WAIT_FOREVER);
        if ((status == CY_U3P_SUCCESS) && (glSibDrvEntry != NULL))
            glSibDrvEntry ();
    }
}
        ...
        void)
{
    ...
    if (ptr != NULL)
    {
        ...
        CyU3PSystemRegisterDriver (&glSibThread);
    }
}
