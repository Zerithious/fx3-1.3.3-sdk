/*
 ## Cypress FX3 Device Firmware Source (cyu3debug.c)
 ## ===========================
 ##
...
static uint32_t         glDbgTimeout = CYU3P_WAIT_FOREVER;

#define CY_U3P_DEBUG_STACK_SIZE	        (0x200)                 /* Debug Thread stack size in bytes */
#define CY_U3P_DEBUG_THREAD_PRIORITY    (6)                     /* Debug Thread Priority */
#define CY_U3P_DEBUG_QUEUE_SIZE         (0x80)                  /* Debug Queue size in bytes */
...
CyU3PDebugApplicationDefine (void)
{
    ...
    CyU3PThreadCreate (&glDebugThread, "07_DEBUG_THREAD", CyU3PDebugThreadEntry, 0, pointer, 
        CY_U3P_DEBUG_STACK_SIZE, CY_U3P_DEBUG_THREAD_PRIORITY, CY_U3P_DEBUG_THREAD_PRIORITY, 
        CYU3P_NO_TIME_SLICE, CYU3P_AUTO_START);
    CyU3PSystemRegisterDriver (&glDebugThread);

    pointer = CyU3PMemAlloc (CY_U3P_DEBUG_QUEUE_SIZE);
    CyU3PQueueCreate (&glDebugQueue, sizeof (CyU3PDebugLog_t) / sizeof (uint32_t), pointer, 
...
CyU3PDebugInit (CyU3PDmaSocketId_t destSckId, uint8_t traceLevel)
{
    ...
    glDebugInit       = CyTrue;
    glDbgTimeout      = CYU3P_WAIT_FOREVER;
...
CyU3PDebugChannelReset (void)
{
    ...
    CyU3PDmaChannelSetXfer (&glDebugChanHandle, 0);

    /* No error checks required here as GetBuffer will always succeed after a channel reset. */
    CyU3PDmaChannelGetBuffer (&glDebugChanHandle, &glDebugBuf_p, CYU3P_NO_WAIT);
}
    ...
    for (;;)
    {
        ...
        if (status == CY_U3P_SUCCESS)
        {
            ...
            if (glDebugBufOffset == CY_U3P_DEBUG_DMA_BUFFER_SIZE - 8)
            {
                ...
                {
                    ret = CyU3PDmaChannelGetBuffer (&glDebugChanHandle, &glDebugBuf_p, glDbgTimeout);
                }
