/*
 ## Cypress FX3 Device Firmware Source (cyu3device.c)
 ## ===========================
 ##
        ...
        CyU3PSysClockConfig_t *clkCfg)
{
    ...
    uint32_t cpuDivider = 2;
    CyBool_t setSysClkDiv = CyFalse;
    uint32_t *regBkp = NULL;
    ...
    /* Calculate the CPU frequency. This is used to calibrate the BusyWait implementation. */
    CyFx3SetCpuFreq (glSysClkFreq / cpuDivider);
    ...
    regBkp = CyU3PSysGetRegBkpAddr ();
    if (regBkp != 0)
        CyU3PLppRestoreState (regBkp);
        ...
        uint32_t period)
{
    ...
    /* Make sure that the WatchDog Timer does not get shut off unexpectedly. */
    if ((VIC->int_enable & (1 << CY_U3P_VIC_WDT_VECTOR)) != 0)
    {
