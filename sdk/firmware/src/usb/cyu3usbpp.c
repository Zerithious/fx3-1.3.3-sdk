/*
 ## Cypress FX3 USB Driver Source (cyu3usbpp.c)
 ## ===========================
 ##
...
#define CY_U3P_UIB_EVT_USB3_LNKFAIL     (1 << 27)
...
/* Number of USB 3.0 connection attempts. */
#define CY_U3P_USB_TDISABLED_LIMIT      (5)
...
static volatile uint32_t glUibSocketSuspended = 0;
...
CyU3PUsbGetEventLogIndex (
        void)
{
    /* We do not disable interrupts while trying to get the current index.
     * This is not required as the worst case side effect would be losing the latest log message.
     */
...
CyU3PUsbAddToEventLog (
        uint8_t data)
{
    /* Disable interrupts while doing the update as this could be called from ISR as well. */
    uint32_t mask = CyU3PVicDisableAllInterrupts ();
        ...
        uint16_t *lnk_error_cnt)
{
    ...
    tmp = USB3LNK->lnk_error_count;
    USB3LNK->lnk_error_count = 0;

    phy_errs = (tmp >> 16) + (glUsbLinkErrorCount >> 16);
    phy_errs = CY_U3P_MIN (phy_errs, 0xFFFF);

    lnk_errs = (tmp & 0xFFFF) + (glUsbLinkErrorCount & 0xFFFF);
    lnk_errs = CY_U3P_MIN (lnk_errs, 0xFFFF);

    glUsbLinkErrorCount = 0;

    *phy_err_cnt   = (uint16_t)phy_errs;
    *lnk_error_cnt = (uint16_t)lnk_errs;
        ...
        void)
{
    ...
    /* Reset the EP-0 DMA channels and enable EP0-In for transfers. */
        ...
        void)
{
    ...
    UIB->dev_epi_cs[0] = (UIB->dev_epi_cs[0] & ~CY_U3P_UIB_EPI_STALL) | CY_U3P_UIB_EPI_VALID;
    UIB->dev_epo_cs[0] = (UIB->dev_epo_cs[0] & ~CY_U3P_UIB_EPO_STALL) | CY_U3P_UIB_EPO_VALID;
    ...
    {
        if (glUibDeviceInfo.tDisabledCount >= CY_U3P_USB_TDISABLED_LIMIT)
        ...
        uint8_t feature)
{
    ...
    switch (feature)
    {
        ...
        case CY_U3P_USB2_FS_TEST_MODE:
            ...
            {
                glUibDeviceInfo.inUsbTestMode = CyTrue;
                isHandled = CyTrue;
            }
            break;
        ...
        uint32_t setupdat1)
{
    ...
    if ((isHandled == CyTrue) && (buf != NULL))
    {
        ...
        if ((status == CY_U3P_SUCCESS) && (count != 0) && (count < wLength) && ((count & (max - 1)) == 0))
        ...
        void)
{
    ...
    else
    {
        ...
        if ((glUibDeviceInfo.enableSS) && (glUibDeviceInfo.tDisabledCount < CY_U3P_USB_TDISABLED_LIMIT))
        {
...
void CyU3PUsbSSConnecthandler (void)
{
    ...
    /* If this is a second connect (USB 2.0 is already ON, or the ssCmdSeen variable has been set);
       delay the connect processing until we are sure link training is happening. This is to avoid
       affecting the USB 2.0 link due to a spurious termination detect. */
    if (((UIB->otg_ctrl & CY_U3P_UIB_DEV_ENABLE) != 0) ||
            ((glUibDeviceInfo.usb2Disable) && (glUibDeviceInfo.ssCmdSeen)))
    {
        ...
        /* If the ISR turned off the USB 3.0 PHY, do nothing and return. */
        if ((UIB->otg_ctrl & CY_U3P_UIB_SSDEV_ENABLE) == 0)
        {
            return;
        }

        /* If we have gone into compliance state, and this is not the first occurence; treat this
           as a transition to SS.Disabled. */
        if (state == CY_U3P_UIB_LNK_STATE_COMP)
        {
            /* If USB 2.0 is already on, we cannot proceed in compliance test mode. */
            if (((UIB->otg_ctrl & CY_U3P_UIB_DEV_ENABLE) != 0) && (!glUibDeviceInfo.ssCmdSeen))
            {
        ...
        /* We have moved into Polling.RxEq. Disable the USB 2.0 PHY, if it is on. */
    ...
    state = CyU3PVicDisableAllInterrupts ();
    CyFx3UsbWritePhyReg (0x1006, 0x0180);   /* ZKR RX_EQ modifications by GML #only for FX3 */
    CyFx3UsbWritePhyReg (0x1024, 0x0080);
    CyFx3UsbWritePhyReg (0x0030, 0x00C0);
    CyFx3UsbWritePhyReg (0x1010, 0x0080);
    CyU3PVicEnableInterrupts (state);
        ...
        void)
{
    ...
    glUsbEvtCb (CY_U3P_USB_EVENT_DISCONNECT, 0);
        ...
        else
        {
            ...
            if (glUibDeviceInfo.usb2Disable)
            {
                ...
                else
                {
                    ...
                    /* This code could be executed from an ISR as well. Sending the LNKFAIL event callback from
                       ISR will limit the user's ability to handle this properly. Make sure that the LNKFAIL event
                       is always sent from a thread. */
                    ...
                    {
                        CyU3PEventSet (&glUibEvent, CY_U3P_UIB_EVT_USB3_LNKFAIL, CYU3P_EVENT_OR);
                    }
                }
            }
            else
...
void CyU3PUibDevCtrlEvtHandler (void)
{
    ...
    if (state & CY_U3P_UIB_SUDAV)
    {
        ...
        /* If this is being handled even before the interrupt is seen, make sure all status variables are updated. */
...
void CyU3PUibProtIntrEvtHandler(void)
{
    ...
    state = USB3PROT->prot_intr;

    /* Handle status interrupt first. */
    if (state & CY_U3P_UIB_STATUS_STAGE)
    {
        if (glUibDeviceInfo.sendStatusEvent)
        {
    ...
    if (state & CY_U3P_UIB_SUTOK_EV)
    {
        if (!glUibEp0StatusPending)
        {
            /* We seemed to have got here before the interrupt was seen. Update all state as required. */
        ...
        CyU3PUsbSetupCommand ();
        USB3PROT->prot_intr_mask |= CY_U3P_UIB_SUTOK_EN;
    }
...
{
    int i;
    uint32_t x, epCs, stat;

    x = USB3PROT->prot_ep_intr & USB3PROT->prot_ep_intr_mask;
    for (i = 1; i < 16; i++)
    {
        /* Check if any of the sockets are suspended. */
        if ((glUibSocketSuspended & (1 << i)) != 0)
        {
            /* Check if HBTERM interrupt is pending on the endpoint. */
            if (((USB3PROT->prot_epi_cs1[i] & CY_U3P_UIB_SSEPI_HBTERM) != 0) ||
                    (((UIB->eepm_cs & CY_U3P_UIB_VALID_PACKETS_MASK) != 0) &&
                     (((UIB->eepm_cs & CY_U3P_UIB_EG_EPNUM_MASK) >> CY_U3P_UIB_EG_EPNUM_POS) == i)))
            {
                if (((glUsbEvtEnabledEps & (1 << i)) != 0) && ((glUsbEpEvtMask & CYU3P_USBEP_SS_RESET_EVT) != 0))
                {
                    glUsbEpCb (CYU3P_USBEP_SS_RESET_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);
                }

                CyU3PUsbResetEndpointMemories ();
                UIB->sck[i].intr    = CY_U3P_UIB_SUSPEND;
                UIB->sck[i].status &= ~CY_U3P_UIB_GO_SUSPEND;
            }
            else
            {
                UIB->sck[i].intr    = CY_U3P_UIB_SUSPEND;
            ...
            glUibSocketSuspended &= ~(1 << i);
        }

        /* IN-EP i */
        if ((x & (1 << i)) != 0)
        {
            /* Extract the endpoint interrupt status from the register. */
            epCs = USB3PROT->prot_epi_cs1[i];
            stat = epCs & 0x0007FF00;
            epCs = epCs & ~(0x0007FF00);

            /* DBTERM handler: Just clear the interrupt. */
            if (stat & CY_U3P_UIB_SSEPI_DBTERM)
            {
                USB3PROT->prot_epi_cs1[i] = epCs | CY_U3P_UIB_SSEPI_DBTERM;
            }

            if ((glUsbEvtEnabledEps & (1 << i)) && (glUsbEpCb != 0))
            {
                if ((glUsbEpEvtMask & CYU3P_USBEP_NAK_EVT) && (stat & CY_U3P_UIB_SSEPI_FLOWCONTROL))
                    glUsbEpCb (CYU3P_USBEP_NAK_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);

                if ((glUsbEpEvtMask & CYU3P_USBEP_ZLP_EVT) && (stat & CY_U3P_UIB_SSEPI_ZERO))
                    glUsbEpCb (CYU3P_USBEP_ZLP_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);

                if ((glUsbEpEvtMask & CYU3P_USBEP_SLP_EVT) && (stat & CY_U3P_UIB_SSEPI_SHORT))
                    glUsbEpCb (CYU3P_USBEP_SLP_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);

                if ((glUsbEpEvtMask & CYU3P_USBEP_SS_STREAMERR_EVT) && (stat & CY_U3P_UIB_SSEPI_STREAM_ERROR))
                    glUsbEpCb (CYU3P_USBEP_SS_STREAMERR_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);

                if ((glUsbEpEvtMask & CYU3P_USBEP_SS_SEQERR_EVT) && (stat & CY_U3P_UIB_SSEPI_OOSERR))
                {
                    glUsbEpCb (CYU3P_USBEP_SS_SEQERR_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);
                }

                if ((glUsbEpEvtMask & CYU3P_USBEP_SS_RETRY_EVT) && (stat & CY_U3P_UIB_SSEPI_RETRY))
                {
                    glUsbEpCb (CYU3P_USBEP_SS_RETRY_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);
                }

                if ((glUsbEpEvtMask & CYU3P_USBEP_SS_BTERM_EVT) && (stat & CY_U3P_UIB_SSEPI_HBTERM))
                {
                    glUsbEpCb (CYU3P_USBEP_SS_BTERM_EVT, CY_U3P_SUPER_SPEED, 0x80 | i);
                }
            }

            /* Clear all EP interrupts. */
            USB3PROT->prot_epi_cs1[i] = epCs | stat;
        }
        ...
        if ((x & (1 << (16 + i))) != 0)
        {
            ...
            if ((glUsbEvtEnabledEps & (1 << (16 + i))) && (glUsbEpCb != 0))
            {
                ...
                if ((glUsbEpEvtMask & CYU3P_USBEP_SS_BTERM_EVT) && (epCs & CY_U3P_UIB_SSEPO_HBTERM))
                {
                    glUsbEpCb (CYU3P_USBEP_SS_BTERM_EVT, CY_U3P_SUPER_SPEED, i);
                }
            }
...
void CyU3PUsbSSReset(void)
{
    ...
    /* Update USB state machine state. */
    if (glUibDeviceInfo.usbState >= CY_U3P_USB_CONNECTED)
        glUibDeviceInfo.usbState = CY_U3P_USB_CONNECTED;
}
        ...
        uint32_t flag)
{
    ...
    {
        /* Filter out underrun callback while in USB 2.0 device test mode. */
        if ((glUsbEvtCb != NULL) && (!glUibDeviceInfo.inUsbTestMode))
        {
    ...
    if (flag & CY_U3P_UIB_EVT_USB3_LNKFAIL)
    {
        if (glUsbEvtCb != NULL)
            glUsbEvtCb (CY_U3P_USB_EVENT_USB3_LNKFAIL, 0);
    }
    ...
    if (flag & CY_U3P_UIB_EVT_TRY_UX_EXIT)
    {
        ...
        {
            /* Do a remote wake because the app does not want to be stuck in LPM-L1. If EP0 transfers are pending,
               the resume will be triggered from a timer. Initiating two resume signals without a time-gap is likely
               to cause a problem. */
            if (!glUibEp0StatusPending)
            {
                CyU3PUsb2Resume ();
...
CyU3PUibStatusTimerCb (
        uint32_t arg)
{
    CyBool_t restart = CyFalse;

    if (glUibEp0StatusPending)
    {
        if (glUibDeviceInfo.usbSpeed == CY_U3P_SUPER_SPEED)
        {
            if (glUibStatusSendErdy)
            {
        ...
        {
            /* Make sure the link is brought out of L1 periodically. */
            glUibDeviceInfo.resumeFromTimer = CyTrue;
            CyU3PUsb2Resume ();
            glUibDeviceInfo.resumeFromTimer = CyFalse;

            restart = CyTrue;
        }
    }

    /* Start the timer again if required to keep the transfer going. */
    if (restart)
    {
        ...
        uint32_t arg)
{
    ...
    /* Clear the 3.0 link error counts after adding the current value to the running count. */
    if (CyU3PUsbGetSpeed() == CY_U3P_SUPER_SPEED)
    {
        tmp = USB3LNK->lnk_error_count;
        USB3LNK->lnk_error_count = 0;

        /* Add the link and phy error counts separately to avoid side-effects due to overflow. If overflow is detected,
           saturate the count at the maximum value. */
        phy_errs = (tmp >> 16) + (glUsbLinkErrorCount >> 16);
        phy_errs = CY_U3P_MIN (phy_errs, 0xFFFF);
        ...
        glUsbLinkErrorCount = (phy_errs << 16) + lnk_errs;
    }
}
        ...
        uint32_t threadInput)
{
    ...
    uibMask = (CY_U3P_EVENT_QUEUE |
            CY_U3P_UIB_EVT_EP_INT | CY_U3P_UIB_EVT_DEV_CTRL_INT |
            ...
            CY_U3P_UIB_EVT_USB3_U3_SUSPEND | CY_U3P_UIB_EVT_USB3_U0_RESUME | CY_U3P_UIB_EVT_UX_REENABLE |
            CY_U3P_UIB_EVT_TRY_UX_EXIT | CY_U3P_UIB_EVT_LNK_ERR_LIMIT |
            CY_U3P_UIB_EVT_EPM_UNDERRUN | CY_U3P_UIB_EVT_USB3_LNKFAIL |
            CY_U3P_UIB_EVT_HOST_INT | CY_U3P_UIB_EVT_HOST_EP_INT | CY_U3P_UIB_EVT_EHCI_INT | CY_U3P_UIB_EVT_OHCI_INT |
            CY_U3P_UIB_EVT_OTG_INT | CY_U3P_UIB_EVT_CHGDET_INT);
        ...
        void)
{
    ...
    if (pointer != NULL)
    {
        ...
        if (status == 0)
        {
            CyU3PSystemRegisterDriver (&glUibThread);
        ...
        void)
{
    ...
    if (state & CY_U3P_UIB_LNK_INT)
    {
        ...
        if (lnkState & CY_U3P_UIB_LTSSM_STATE_CHG)
        {
            ...
            if (USB3LNK->lnk_error_status & CY_U3P_UIB_HP_TIMEOUT_EV)
            {
                ...
                CyU3PUsbAddToEventLog (CYU3P_USB_LOG_USB_HP_TIMEOUT);
            ...
            /* If we have seen too many errors since the last timer interrupt, reset the USB link. */
            if ((USB3LNK->lnk_error_count & CY_U3P_UIB_LINK_ERROR_COUNT_MASK) >= CY_U3P_USBLNK_ERRCNT_LIMIT)
            {
                CyU3PUsbAddToEventLog (CYU3P_USB_LOG_USBSS_LNKFAIL);
            ...
            switch (tmp)
            {
                case CY_U3P_UIB_LNK_STATE_SSINACT_QUT:
                case CY_U3P_UIB_LNK_STATE_SSINACT_DET:
                    CyU3PUsbAddToEventLog (CYU3P_USB_LOG_USBSS_INACTIVE);
                    break;
        ...
        {
            /* Re-enable LMP PORT_CAP and LMP PORT_CFG. */
            USB3PROT->prot_lmp_port_capability_timer    = CY_U3P_UIB_PROT_LMP_PORT_CAP_TIMER_VALUE;
            USB3PROT->prot_lmp_port_configuration_timer = CY_U3P_UIB_PROT_LMP_PORT_CFG_TIMER_VALUE;
    ...
    if (state & CY_U3P_UIB_PROT_INT)
    {
        ...
        {
            /* Force link into SS.Disabled state immediately. */
            USB3LNK->lnk_ltssm_state = CY_U3P_UIB_LTSSM_OVERRIDE_GO;
            USB3PROT->prot_intr      = 0xFFFFFFFF; /* Clear all Protocol interrupts. */
            CyU3PUsbSSDisConnecthandler ();

            CyU3PUsbAddToEventLog (CYU3P_USB_LOG_PORTCAP_TIMEOUT);
        }
        if (devState & CY_U3P_UIB_LMP_PORT_CAP_EV)
        {
            CyU3PUsbAddToEventLog (CYU3P_USB_LOG_PORTCAP_DONE);
        ...
        {
            CyU3PUsbAddToEventLog (CYU3P_USB_LOG_PORTCFG_DONE);
        ...
        if (devState & CY_U3P_UIB_SUTOK_EV)
        {
            ...
            USB3PROT->prot_intr_mask &= ~CY_U3P_UIB_SUTOK_EV;
        }
    ...
    if (state & CY_U3P_UIB_DEV_CTL_INT)
    {
        ...
        {
            /* Clear stall on EP0. */
            UIB->dev_epi_cs[0] = (UIB->dev_epi_cs[0] & ~CY_U3P_UIB_EPI_STALL);
            UIB->dev_epo_cs[0] = (UIB->dev_epo_cs[0] & ~CY_U3P_UIB_EPO_STALL);
        ...
        if (devState & CY_U3P_UIB_SUSP)
        {
            ...
            {
                glUibDeviceInfo.l1EntrySeen = CyTrue;

                UIB->phy_chirp  = CY_U3P_UIB_OVERRIDE_FSM | 0x10;
                UIB->dev_pwr_cs = (UIB->dev_pwr_cs & ~CY_U3P_UIB_SIGRSUME) | CY_U3P_UIB_DEV_SUSPEND;

                /* Enable wake-up interrupts on D+/D- Change. */
                GCTLAON->wakeup_event = 0x3FFF;
                CyU3PBusyWait (20);

                GCTLAON->wakeup_en = CY_U3P_GCTL_EN_UIB_DP;
                ...
                /* If we have a pending control request, or if LPM is disabled by app, set an event requesting
                   for remote wake. */
                if (glUibDeviceInfo.isLpmDisabled)
                    CyU3PEventSet (&glUibEvent, CY_U3P_UIB_EVT_TRY_UX_EXIT, CYU3P_EVENT_OR);
            }
    ...
    if (state & CY_U3P_UIB_PROT_EP_INT)
    {
        ...
        {
            uint32_t tmp = USB3PROT->prot_ep_intr & USB3PROT->prot_ep_intr_mask;
            uint32_t y;
            ...
            if ((tmp & 0xFFFE) != 0)
            {
                for (y = 1; y < 16; y++)
                {
                    if ((tmp & (1 << y)) != 0)
                    {
                        if ((USB3PROT->prot_epi_cs1[y] & CY_U3P_UIB_SSEPI_RETRY) != 0)
                        {
                            /* Set the NRDY bit and suspend the socket. */
                            glUibSocketSuspended |= (1 << y);
                            UIB->sck[y].status   |= CY_U3P_UIB_GO_SUSPEND;
                        }
                    }
                }
            }
    ...
    if (state & CY_U3P_UIB_EPM_URUN)
    {
        ...
        CyU3PUsbAddToEventLog (CYU3P_USB_LOG_EP_UNDERRUN);
        UIB->intr_mask &= ~CY_U3P_UIB_EPM_URUN;
